...
package cl.bice.coe.bamboospecs;

import cl.bice.coe.bamboospecs.models.BuildModel;
...
public class BuildControl extends BambooController {
    ...
    public void run(UserPasswordCredentials adminUser, File yamlFile, boolean publish) {
        ...
        LOGGER.info("Parseando YAML {}", yamlFile.toPath());
        ...
        try {
            ...
            if (!violations.isEmpty()) {
                ...
                throw new RuntimeException("El archivo YAML (" + yamlFile.getName() + ") falló la validación. Ver mensage(s): " + allErrors);
            }
        } catch (Exception e) {
            throw new RuntimeException("Error leyendo archivo YAML: " + e.getMessage(), e);
        }
        ...
        if (publish) {
            ...
            LOGGER.info("YAML parseado correctamente OK");
        }
    }
    ...
    private void publishLabels(BuildModel yamlPlan, UserPasswordCredentials adminUser) throws IOException {
        ...
        for (String label : yamlPlan.labels) {
            ...
            if (!response.isSuccessful()) {
                throw new RuntimeException("Error al publicar labels. La llamada HTTP fue erronea debido a: " + response.message());
            } else {
                LOGGER.info("Agregado label {} al build {}-{}", label, yamlPlan.projectKey, yamlPlan.planKey);
            }
        }
    }
